...
class Produtos(ctk.CTkFrame):
    ...
    def carregar_coluna1(self):
        ...
        for index, categoria in enumerate(self.categorias):
            btn_categoria = ctk.CTkButton(col1, text=categoria, height=35, command=lambda t=categoria: atualizar_produtos(t))
            btn_categoria.grid(row=index + 1, column=0, sticky="nsew", pady=2, padx=15)
    ...
    def carregar_coluna2(self):
        ...
        col2_frame_scroll = ctk.CTkScrollableFrame(self, orientation="vertical", corner_radius=10, border_width=2, border_color=colors.AZUL_SECUNDARIO, )
        col2_frame_scroll.grid(row=0, column=2, sticky="nsew", padx=5, pady=5)
        col2_frame_scroll.grid_columnconfigure(0, weight=1)
        # col2_frame_scroll.grid_rowconfigure(0, weight=1)

        for index, (nome_produto, dados_produto) in enumerate(self.produtos.items()):
            frame_produto = ctk.CTkFrame(col2_frame_scroll)
            frame_produto.grid(row=index, column=0, sticky="nsew", pady=15, padx=15)
